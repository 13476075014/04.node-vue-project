...
//路径定义
var srcDir = path.resolve(__dirname, './public'); //获取所有静态资源文件绝对路径
...
var publicPath = '/js/'; //设置的打包后的静态资源引入的前缀地址，因为打包后的静态文件是放到dist下面的js,然后在node中设置了静态访问地址dist,所以这里直接 /js/..就可以访问到这些文件
//插件定义
...
var html_plugins = function () {
    var entryHtml = glob.sync(viewDir + '/*.html')
    var r = []
...
function webpackConfig(options){
    ...
    if(debug){
        config.devServer={
            ...
            //     ],
            //   },
            contentBase:  path.join(__dirname, "dist"), //要打开的目录
            // publicPath:'./',
            // contentBase:  false, //要打开的目录
            host:'localhost',
            port:7888,//控制端口
            // compress: true,//压缩
